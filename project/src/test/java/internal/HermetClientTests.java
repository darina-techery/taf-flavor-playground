...
public class HermetClientTests extends BaseTest {
	...
	public void setupHermetSession(){
		...
		hermetApi = client.create(HermetAPI.class);
	}
	...
	@Test
	public void test_deleteSession() throws IOException {
		String anotherTargetUrl = "www.sometesturl.com";
		String anotherServiceId = HermetServiceManager.getServiceId(anotherTargetUrl);
		actions.removeService(anotherTargetUrl);

		Response<List<HermetProxyData>> activeSessions = hermetApi.getActiveServices().execute();
		HermetProxyData serviceData = activeSessions.body().stream()
				.filter(service -> service.getId().equals(anotherServiceId))
				.findAny().orElse(null);
		Assert.assertThat("Service for "+anotherTargetUrl+" was deleted",
				serviceData, is(nullValue()));
	}
